...
export async function getReadingsOrderedByUpdatedAt(): Promise<
...
> {
  try {
    ...
    for (const reading of readings) {
      ...
      if (questionsError) {
        console.error(
          `Error fetching questions for reading ${reading.id}:`,
          questionsError
        );
      }
...
export async function getDailyReading(): Promise<ReadingWithQuestions | null> {
  try {
    ...
    const selectedReading = readings[0];
    ...
    if (questionsError) {
      console.error(
        `Error fetching questions for reading ${selectedReading.id}:`,
        questionsError
      );
    }

    // Procesar las preguntas para incluir las opciones
    const processedQuestions: QuestionWithOptions[] = (questions || []).map(
      (q) => {
        ...
        return {
          ...
          correct_answer:
            q.correct_answer ||
            options.find((o: any) => o.is_correct)?.option_text ||
            "",
          options,
        };
      }
